...
import { AnyRecord, Crud, CrudStorage, PrimaryKeyFillStrategy, SafeKeyTypes, TableControllerOptions } from './types.ts'

const HASH_PRIMARY_KEY_LENGTH = 24

const __DEFAULT_BASE_STORAGE: CrudStorage = {}

const exists = <T>(undefinable: T | undefined): undefinable is T => undefinable !== undefined
...
export const useTableWithStore =
    ...
    <T extends Record<string, unknown>, PK extends SafeKeyTypes<T>>(
        ...
        return {
            ...
            create(item) {
                const index = options?.pkStrategy === PrimaryKeyFillStrategy.AutoIncrement
                    ? lastId++
            ...
            find(reducer) {
                return Object.entries(STORAGE[tableName]).find(([key, value]) => reducer(value, key as PK))?.[1]
            },
            findAll(reducer) {
            ...
            updateBatch(
                externalItems: Record<PK, T[PK]>[],
            ) {
                return externalItems.map((extElement) =>
                    extElement[pk] !== undefined ? { ...(this.get(extElement[pk]) || {}), ...extElement } : extElement
                )
            },
            _merge<ExternalType = Record<string, any>>(
                ...
                return externalItems.map((extElement) =>
                    extElement[pk] !== undefined ? { ...extElement, ...(this.get(extElement[pk]) || {}) } : extElement
                )
            },
...
export const getBoundStorage = (storage: CrudStorage) => useTableWithStore(storage)
